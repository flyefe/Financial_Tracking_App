...
def home():
    ...
    total_income_today = sum(transaction.amount for transaction in transactions
                             ...
                             and transaction.transaction_date.date() == today)
    ...
    total_income_last_7_days = sum(
        transaction.amount for transaction in transactions
        if transaction.transaction_type == 'income'
        and seven_days_ago <= transaction.transaction_date.date() <= today)
    total_expenses_last_7_days = sum(
        transaction.amount for transaction in transactions
        if transaction.transaction_type == 'expense'
        and seven_days_ago <= transaction.transaction_date.date() <= today)
    ...
    if request.method == 'POST':
        ...
        if transction_type == 'expense' and float(amount) > balance:
            flash(
                f'your have ₦{balance} left. want to add more funds to yourfundtracker?'
            )
        else:
            ...
            if transction_type == 'income':
                flash(
                    f'Congratulations {current_user.first_name}!!! ₦{amount} added successfully. Remember your your tracker when you spend from {description}. Good Job.😊',
                    category='success')
            else:
                flash(
    ...
    return render_template(
        ...
        total_expenses_last_7_days=total_expenses_last_7_days,
